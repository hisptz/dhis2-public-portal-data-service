...
function RouteComponent() {
    ...
    const { configId } = useParams({
        from: '/metadata-migrations/_provider/$configId/',
    })
    ...
    const { loading, runs, fetching, pagination, error, refetch } =
        useConfigurationRuns('metadata', configId)

    return (
        <div className="h-full w-full flex flex-col gap-4">
            <div>
                <Button
                    onClick={() => navigate({ to: '/metadata-migrations' })}
                    ...
                    config ? (
                        <ButtonStrip>
                            <RunConfiguration
                                ...
                                preselectedService="metadata-migration"
                            />
                        </ButtonStrip>
                    ) : undefined
                }
